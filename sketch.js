...
function wheelSketch(_p5) {
    ...
    button.mousePressed(function () {
      if (!isCounterAnimation) {
        ...
        const duration = 22000,
          ...
          totalRows =
            data.length * circlesCountForDataLength() + randomKey - correction;
        ...
        const moveSecondAnimUpOrDown =
          Math.random() < 0.5 ? counter - 50 : counter + 50;
        const secondAnimEndPos =
          moveSecondAnimUpOrDown + height_str * totalRows;
        const moveThirddAnimUpOrDown =
          Math.random() < 0.5 ? counter - 800 : counter + 800;
  ...
  _p5.draw = () => {
    ...
    for (let i = -data.length - 2; i < itemsPerScreen + 1; i++) {
      ...
      _p5.fill(
        255,
        Math.round(_p5.map(x + 50, centerX, centerX + radius, 0, 255, true))
      );
      let key = 0;
      try {
